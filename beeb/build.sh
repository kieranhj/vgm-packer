...
echo "== sim: decoder =="
"$BEEBASM" -i player.asm -D TEST=1 -d -labels labels.txt
python sim_test.py

echo "== sim: full player path =="
"$BEEBASM" -i player.asm -D TEST=0 -d -labels labels_full.txt
python sim_test_player.py
python measure_cycles.py

echo "== build bootable disc =="
rm -f music.ssd
"$BEEBASM" -i player.asm -D TEST=0 -do music.ssd -boot Player -title GHOST -opt 3
